...
const char* ssid = "xitrme"; // имя вашей сети
const char* password = "350jianSF"; // пароль вашей сети

IPAddress Ip(10,0,1,51); // IP-адрес для ESP
IPAddress Gateway(10,0,1,1); // IP-адрес шлюза (роутера)
IPAddress Subnet(255,255,255,0); // маска подсети, диапазон IP-адресов в локальной сети
 
#define LED_COUNT 32 // number of pixels in the tape
#define LED_DT 3    //pin where is connected the DIN strips (number of pins coincides with the ESP8266 Arduino) 

uint8_t bright = 200; // bright (0 - 255)
uint8_t ledMode = 0; //rigger effect (0 - 29)
